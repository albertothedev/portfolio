  ...
  const projects: Array<TProject> = [
    {
      ...
      links: {
        demo: process.env.REACT_APP_MINESWEEPER_UI_URL,
        source: process.env.REACT_APP_MINESWEEPER_SOURCE_URL,
      },
    },
    {
      name: "Loldata",
      image: loldata,
      shortDesc: "Stats and analytics app built using React, Express, and the Riot API.",
      longDesc:
      ...
      links: {
        demo: process.env.REACT_APP_LOLDATA_UI_URL,
        source: process.env.REACT_APP_LOLDATA_SOURCE_URL,
      },
    },
    {
      name: "Taskout",
      ...
      links: {
        demo: process.env.REACT_APP_TASKOUT_UI_URL,
        source: process.env.REACT_APP_TASKOUT_SOURCE_URL,
      },
    },
    {
      name: "Snake",
      ...
      links: {
        demo: process.env.REACT_APP_SNAKE_UI_URL,
        source: process.env.REACT_APP_SNAKE_SOURCE_URL,
      },
    },
  ];
    ...
    <div className="projects">
      <div className="projects__header">
        ...
        <p className="projects__header__description">
          The frontends are stored on S3 buckets and delivered by CloudFront to a custom subdomain. The backends are cloned and built on an EC2
          instance managed with EBS where they run continuously on different ports. They use Docker for local development which creates containers for
          the frontend and backend and a database when necessary.
        </p>
      </div>

      <div className="projects__cards">
        {projects.map(
          (project: TProject, index: number): JSX.Element => (
            <div className="projects__cards__card" onClick={() => setSelectedProject(project)} key={index}>
              <img className="projects__cards__card__preview" src={project.image} alt="Picture" />
              <h3 className="projects__cards__card__name">{project.name}</h3>
              <p className="projects__cards__card__description">{project.shortDesc}</p>
            </div>
          )
        )}
      </div>
